...
Class Master extends DBConnection {
	...
	function join_debate() {
		if (empty($_POST['post_id'])) {
			return json_encode([
				'status' => 'failed',
				'msg' => 'Post ID is required to join the debate.'
			]);
		}
	
		$post_id = $this->conn->real_escape_string($_POST['post_id']);
		$user_id = $this->settings->userdata('id');
	
		// Check if the user has already joined the debate
		$check = $this->conn->query("SELECT * FROM debate_rounds WHERE post_id = '{$post_id}' AND user_id = '{$user_id}'");
	
		if ($check->num_rows > 0) {
			return json_encode([
				'status' => 'failed',
				'msg' => 'You have already joined this debate.'
			]);
		}
	
		// Insert the participant into debate_rounds
		$sql = "INSERT INTO debate_rounds (post_id, user_id, round_number, content) VALUES ('{$post_id}', '{$user_id}', 0, '')";
		$save = $this->conn->query($sql);
	
		if ($save) {
			return json_encode([
				'status' => 'success',
				'msg' => 'Successfully joined the debate.'
			]);
		} else {
			return json_encode([
				'status' => 'failed',
	...
	function submit_argument() {
		if (empty($_POST['post_id']) || empty($_POST['content'])) {
			return json_encode([
				'status' => 'failed',
				'msg' => 'Post ID and content are required.'
			]);
		}
		...
		// Get the highest round number for this debate
		$highest_round_query = $this->conn->query("SELECT MAX(round_number) as highest_round FROM debate_rounds WHERE post_id = '{$post_id}'");
		$highest_round = $highest_round_query->fetch_assoc()['highest_round'] ?? 0;
	
		// Check for the round limit from the request
		$round_limit = isset($_POST['round_limit']) ? (int)$_POST['round_limit'] : 0;
	
		// Enforce the round limit
		if ($round_limit > 0 && $highest_round >= $round_limit) {
			return json_encode([
				'status' => 'failed',
				'msg' => 'The debate has reached the maximum number of rounds.'
			]);
		}
		...
		$last_round_query = $this->conn->query("SELECT user_id FROM debate_rounds WHERE post_id = '{$post_id}' AND round_number = '{$highest_round}'");
		$last_user_id = $last_round_query->fetch_assoc()['user_id'] ?? null;
	
		if ($last_user_id == $user_id) {
			return json_encode([
				'status' => 'failed',
				'msg' => 'It is not your turn to submit an argument.'
			]);
		}
	
		// Insert the new argument
		$sql = "INSERT INTO debate_rounds (post_id, user_id, round_number, content) 
				VALUES ('{$post_id}', '{$user_id}', '{$next_round}', '{$content}')";
		$save = $this->conn->query($sql);
	
		if ($save) {
			return json_encode([
				'status' => 'success',
				'msg' => 'Argument submitted successfully.',
...
switch ($action) {
	...
	case 'submit_argument':
		echo $Master->submit_argument();
		break;
	case 'join_debate':
			echo $Master->join_debate();
			break;
